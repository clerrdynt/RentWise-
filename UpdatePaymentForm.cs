...
namespace RentWise_
{
    public partial class UpdatePaymentForm : Form
    {
        ...
        public UpdatePaymentForm(int renterID)
        {
            InitializeComponent();
        ...
        private void btnUpdateProofPayment_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand getPropCmd = new OleDbCommand("SELECT PropertyID FROM Room WHERE RenterID = ?", conn))
                    {
        ...
        private void LoadDormitoryPaymentDetails()
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                ...
                using (OleDbCommand cmd = new OleDbCommand("SELECT PropertyID FROM Room WHERE RenterID = ?", conn))
                {
                ...
                using (OleDbCommand cmd = new OleDbCommand("SELECT PaymentMethod FROM Dormitory WHERE PropertyID = ?", conn))
                {
                    ...
                    {
                        MessageBox.Show("This form is only for Dormitory renters.");
                        return;
                    }
                }
